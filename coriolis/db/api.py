...
def _update_sqlalchemy_object_fields(obj, updateable_fields, values_to_update):
    ...
    non_updateable_fields = set(values_to_update.keys()).difference(
...
def _get_transfer_schedules_filter(
    context, transfer_id=None, schedule_id=None, expired=True
):
    ...
    if schedule_id:
        sched_filter = sched_filter.filter(models.TransferSchedule.id == schedule_id)
    if not expired:
        sched_filter = sched_filter.filter(
            or_(
                models.TransferSchedule.expiration_date == null(),
...
def get_endpoint(context, endpoint_id):
    ...
    if is_user_context(context):
        q = q.filter(models.Endpoint.project_id == context.project_id)
    return q.filter(models.Endpoint.id == endpoint_id).first()
...
def update_endpoint(context, endpoint_id, updated_values):
    ...
    def _try_unmap_regions(region_ids):
        for region_to_unmap in region_ids:
            try:
                ...
                delete_endpoint_region_mapping(context, endpoint_id, region_to_unmap)
            except Exception:
                LOG.warn(
    ...
    if desired_region_mappings is not None:
        ...
        existing_region_mappings = [
            mapping.region_id
            for mapping in get_region_mappings_for_endpoint(context, endpoint_id)
        ]

        # check and add new mappings:
        to_map = set(desired_region_mappings).difference(set(existing_region_mappings))
        regions_to_unmap = set(existing_region_mappings).difference(
            set(desired_region_mappings)
        )
    ...
    try:
        _update_sqlalchemy_object_fields(endpoint, updateable_fields, updated_values)
    except Exception:
        LOG.warn(
            "Exception occurred while updating fields of endpoint '%s'. "
            "Cleaning "
            "up created mappings (%s). Error was: %s",
            endpoint_id,
            newly_mapped_regions,
            utils.get_exception_details(),
        )
...
def delete_endpoint(context, endpoint_id):
    ...
    count = (
        _soft_delete_aware_query(context, models.Endpoint)
        .filter_by(**args)
        .soft_delete()
    )
    if count == 0:
...
def get_transfer_tasks_executions(
    ...
    if marker:
        try:
            marker = get_transfer_tasks_execution(context, transfer_id, marker)
...
def get_transfer_tasks_execution(
    context, transfer_id, execution_id, include_task_info=False, to_dict=False
):
    q = _soft_delete_aware_query(context, models.TasksExecution).join(models.Transfer)
    if include_task_info:
        q = q.options(orm.joinedload('action').undefer('info'))
    q = _get_tasks_with_details_options(q)
    if is_user_context(context):
...
def add_transfer_tasks_execution(context, execution):
    ...
    max_number = (
        _model_query(context, func.max(models.TasksExecution.number))
        .filter(models.TasksExecution.action_id == (execution.action.id))
        .first()[0]
        or 0
    )
...
def delete_transfer_tasks_execution(context, execution_id):
    ...
    if is_user_context(context):
        if (
            not q.join(models.Transfer)
            .filter(models.Transfer.project_id == context.project_id)
            .first()
        ):
            raise exception.NotAuthorized()
    count = q.soft_delete()
...
def get_transfer_schedule(context, transfer_id, schedule_id, expired=True):
    sched_filter = _get_transfer_schedules_filter(
        context, transfer_id=transfer_id, schedule_id=schedule_id, expired=expired
    )
...
@enginefacade.reader
def get_transfer(
    context, transfer_id, transfer_scenario=None, include_task_info=False, to_dict=False
):
    q = _soft_delete_aware_query(context, models.Transfer)
    if include_task_info:
        q = q.options(orm.undefer('info'))
    ...
    transfer = q.filter(models.Transfer.id == transfer_id).first()
    if to_dict and transfer is not None:
        return transfer.to_dict(
            include_task_info=include_task_info, include_executions=False
        )
...
@enginefacade.reader
def get_endpoint_transfers_count(context, endpoint_id, transfer_scenario=None):
    ...
    q_origin_count = (
        _soft_delete_aware_query(context, models.Transfer)
        .filter_by(**origin_args)
        .count()
    )
...
def _delete_transfer_action(context, cls, id):
    ...
    if is_user_context(context):
        args["project_id"] = context.project_id
    count = _soft_delete_aware_query(context, cls).filter_by(**args).soft_delete()
    if count == 0:
        raise exception.NotFound("0 entries were soft deleted")
...
def get_transfer_deployments(context, transfer_id):
    ...
    if is_user_context(context):
        q = q.filter(models.Deployment.project_id == context.project_id)
    return q.filter(models.Transfer.id == transfer_id).all()
...
def get_deployments(
    ...
    if to_dict:
        return [
            i.to_dict(include_task_info=include_task_info, include_tasks=include_tasks)
            for i in result
        ]
    return result
...
def _get_tasks_with_details_options(query):
    return (
        query.options(orm.joinedload("action"))
        .options(orm.joinedload("tasks").joinedload("progress_updates"))
        .options(orm.joinedload("tasks").joinedload("events"))
    )
...
def _get_deployment_task_query_options(query):
    return (
        query.options(
            ...
            .joinedload("tasks")
            .joinedload("progress_updates")
        )
        .options(orm.joinedload("executions").joinedload("tasks").joinedload("events"))
        .options(orm.joinedload("executions").joinedload("action"))
    )


@enginefacade.reader
def get_deployment(context, deployment_id, include_task_info=False, to_dict=False):
    q = _soft_delete_aware_query(context, models.Deployment)
    q = _get_deployment_task_query_options(q)
    if include_task_info:
        q = q.options(orm.undefer('info'))
...
def add_deployment(context, deployment):
    ...
    deployment.project_id = context.project_id or deployment.transfer.project_id
...
@enginefacade.writer
def set_execution_status(context, execution_id, status, update_action_status=True):
    ...
    execution = execution.filter(models.TasksExecution.id == execution_id).first()
    if not execution:
        raise exception.NotFound("Tasks execution not found: %s" % execution_id)

    execution.status = status
    if update_action_status:
        set_action_last_execution_status(context, execution.action_id, status)
    return execution
...
@enginefacade.writer
def set_action_last_execution_status(context, action_id, last_execution_status):
...
def _get_task(context, task_id):
    task = _soft_delete_aware_query(context, models.Task).filter_by(id=task_id).first()
    if not task:
...
def _get_last_task_event(context, task_id):
    q = _soft_delete_aware_query(context, models.TaskEvent)
    last_event = (
        q.filter(models.TaskEvent.task_id == task_id)
        .order_by(models.TaskEvent.index.desc())
        .first()
    )
    return last_event
...
def _get_last_task_progress_update(context, task_id):
    ...
    last_update = (
        q.filter(models.TaskProgressUpdate.task_id == task_id)
        .order_by(models.TaskProgressUpdate.index.desc())
        .first()
    )
    return last_update
...
def _get_last_minion_pool_event(context, pool_id):
    q = _soft_delete_aware_query(context, models.MinionPoolEvent)
    last_event = (
        q.filter(models.MinionPoolEvent.pool_id == pool_id)
        .order_by(models.MinionPoolEvent.index.desc())
        .first()
    )
    return last_event
...
def _get_last_minion_pool_progress_update(context, pool_id):
    ...
    last_event = (
        q.filter(models.MinionPoolProgressUpdate.pool_id == pool_id)
        .order_by(models.MinionPoolProgressUpdate.index.desc())
        .first()
    )
    return last_event
...
def _get_minion_pool_progress_update(context, pool_id, index):
    ...
    return q.filter(
        ...
        models.MinionPoolProgressUpdate.index == index,
    ).first()
...
def add_minion_pool_progress_update(
    ...
    last_progress_update = _get_last_minion_pool_progress_update(context, pool_id)
...
def _get_progress_update(context, task_id, index):
    ...
    return q.filter(
        ...
        models.TaskProgressUpdate.index == index,
    ).first()


@enginefacade.writer
def add_task_progress_update(context, task_id, message, initial_step=0, total_steps=0):
...
def update_transfer(context, transfer_id, updated_values):
    ...
    for field in updateable_fields:
        if mapped_info_fields.get(field, field) in updated_values:
            ...
            setattr(
                transfer, field, updated_values[mapped_info_fields.get(field, field)]
            )

    non_updateable_fields = set(updated_values.keys()).difference(
        {mapped_info_fields.get(field, field) for field in updateable_fields}
    )
    if non_updateable_fields:
        LOG.warn(
...
def get_region(context, region_id):
    ...
    q = q.options(orm.joinedload('mapped_endpoints'))
    q = q.options(orm.joinedload('mapped_services'))
    return q.filter(models.Region.id == region_id).first()


@enginefacade.writer
def update_region(context, region_id, updated_values):
    if not region_id:
        raise exception.InvalidInput("No region ID specified for updating.")
    region = get_region(context, region_id)
    if not region:
        raise exception.NotFound("Region with ID '%s' does not exist." % region_id)

    updateable_fields = ["name", "description", "enabled"]
    _update_sqlalchemy_object_fields(region, updateable_fields, updated_values)
...
def delete_region(context, region_id):
    region = get_region(context, region_id)
    count = (
        _soft_delete_aware_query(context, models.Region)
...
def get_endpoint_region_mapping(context, endpoint_id, region_id):
    ...
    q = q.filter(models.EndpointRegionMapping.region_id == region_id)
    q = q.filter(models.EndpointRegionMapping.endpoint_id == endpoint_id)
    return q.all()
...
def delete_endpoint_region_mapping(context, endpoint_id, region_id):
    ...
    count = (
        _soft_delete_aware_query(context, models.EndpointRegionMapping)
        .filter_by(**args)
        .delete()
    )
    if count == 0:
...
@enginefacade.reader
def get_region_mappings_for_endpoint(context, endpoint_id, enabled_regions_only=False):
    q = _soft_delete_aware_query(context, models.EndpointRegionMapping)
    q = q.join(models.Region)
    q = q.filter(models.EndpointRegionMapping.endpoint_id == endpoint_id)
    if enabled_regions_only:
        q = q.filter(models.Region.enabled == True)  # noqa: E712
    return q.all()
...
def get_mapped_endpoints_for_region(context, region_id):
    ...
    q = q.filter(models.EndpointRegionMapping.endpoint_id == region_id)
    return q.all()
...
def get_service(context, service_id):
    ...
    return q.filter(models.Service.id == service_id).first()
...
def find_service(context, host, binary, topic=None):
    ...
    if topic:
        args["topic"] = topic
    q = (
        _soft_delete_aware_query(context, models.Service)
        .options(orm.joinedload('mapped_regions'))
        .filter_by(**args)
    )
    return q.first()
...
def update_service(context, service_id, updated_values):
    if not service_id:
        raise exception.InvalidInput("No service ID specified for updating.")
    ...
    def _try_unmap_regions(region_ids):
        for region_to_unmap in region_ids:
            try:
                ...
                delete_service_region_mapping(context, service_id, region_to_unmap)
            except Exception:
                LOG.warn(
                    "Exception occurred while attempting to unmap region '%s' "
    ...
    if desired_region_mappings is not None:
        ...
        for region_id in desired_region_mappings:
            ...
            if not region:
                raise exception.NotFound(
                    "Could not find region with ID '%s' for associating "
                    "with serce '%s' during update process." % (region_id, service_id)
                )

        # get all existing mappings:
        existing_region_mappings = [
            mapping.region_id
            for mapping in get_region_mappings_for_service(context, service_id)
        ]
    ...
    try:
        _update_sqlalchemy_object_fields(service, updateable_fields, updated_values)
    except Exception:
        LOG.warn(
...
def get_service_region_mapping(context, service_id, region_id):
    ...
    q = q.filter(models.ServiceRegionMapping.region_id == region_id)
    q = q.filter(models.ServiceRegionMapping.service_id == service_id)
    return q.all()
...
def delete_service_region_mapping(context, service_id, region_id):
    ...
    # `soft_delete()` util we find a better option:
    count = (
        _soft_delete_aware_query(context, models.ServiceRegionMapping)
        .filter_by(**args)
        .delete()
    )
    if count == 0:
...
@enginefacade.reader
def get_region_mappings_for_service(context, service_id, enabled_regions_only=False):
    q = _soft_delete_aware_query(context, models.ServiceRegionMapping)
    q = q.join(models.Region)
    q = q.filter(models.ServiceRegionMapping.service_id == service_id)
    if enabled_regions_only:
...
def get_mapped_services_for_region(context, region_id):
    ...
    q = q.filter(models.ServiceRegionMapping.service_id == region_id)
    return q.all()
...
def get_minion_machines(context, allocated_action_id=None):
    ...
    if allocated_action_id:
        q = q.filter(models.MinionMachine.allocated_action == allocated_action_id)
    return q.all()
...
def get_minion_machine(context, minion_machine_id):
    q = _soft_delete_aware_query(context, models.MinionMachine)
    return q.filter(models.MinionMachine.id == minion_machine_id).first()


@enginefacade.writer
def update_minion_machine(context, minion_machine_id, updated_values):
    if not minion_machine_id:
        raise exception.InvalidInput("No minion_machine ID specified for updating.")
    ...
    _update_sqlalchemy_object_fields(minion_machine, updateable_fields, updated_values)
...
def set_minion_machines_allocation_statuses(
    ...
    existing_machine_id_mappings = {machine.id: machine for machine in machines}
    missing = [
        mid for mid in minion_machine_ids if mid not in existing_machine_id_mappings
    ]
    if missing:
        raise exception.NotFound(
            "The following minion machines could not be found: %s" % (missing)
        )
...
def delete_minion_machine(context, minion_machine_id):
    ...
    count = (
        _soft_delete_aware_query(context, models.MinionMachine)
        .filter_by(id=minion_machine_id)
        .delete()
    )
    if count == 0:
...
def delete_minion_pool(context, minion_pool_id):
    ...
    if is_user_context(context):
        args["project_id"] = context.project_id
    count = (
        _soft_delete_aware_query(context, models.MinionPool)
        .filter_by(**args)
        .soft_delete()
    )
    if count == 0:
...
def get_minion_pool(
    ...
    return q.filter(models.MinionPool.id == minion_pool_id).first()
...
def set_minion_pool_status(context, minion_pool_id, status):
    pool = get_minion_pool(context, minion_pool_id, include_machines=False)
    if not pool:
        raise exception.NotFound("Minion pool '%s' not found" % minion_pool_id)
    LOG.debug(
...
def update_minion_pool(context, minion_pool_id, updated_values):
    lifecycle = get_minion_pool(context, minion_pool_id, include_machines=False)
    if not lifecycle:
        raise exception.NotFound("Minion pool '%s' not found" % minion_pool_id)
    ...
    non_updateable_fields = set(updated_values.keys()).difference(updateable_fields)
    if non_updateable_fields:
        LOG.warn(
    ...
    if sort_dirs:
        ...
        for sort_dir in sort_dirs:
            if sort_dir not in ('asc', 'desc'):
                msg = f"Unknown sort direction: {sort_dir}, must be 'desc' or 'asc'."
