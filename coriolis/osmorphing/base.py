...
class BaseLinuxOSMorphingTools(BaseOSMorphingTools):
    ...
    def _parse_version_util(cls, version: str) -> Version:
        ...
        try:
            ...
            raise ValueError(
                f"Could not parse version from release string: {version!r}"
            ) from exc
    ...
    def run_user_script(self, user_script):
        ...
        try:
            utils.exec_ssh_cmd(
                self._conn, "sudo chmod +x %s" % script_path, get_pty=False
            )
    ...
    def _write_file_sudo(self, chroot_path, content):
        ...
        utils.exec_ssh_cmd(self._ssh, "sudo sync", self._environment, get_pty=False)
                ...
                if mac_address and mac_address == nic_mac:
                    if new_nic_mac != nic_mac:
                        raise exception.CoriolisException(
                            ...
                            "configuration." % (nic.get('name'), nic_mac, new_nic_mac)
                        )
                    LOG.info(
