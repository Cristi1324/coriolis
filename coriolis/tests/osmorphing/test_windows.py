...
class BaseWindowsMorphingToolsTestCase(test_base.CoriolisBaseTestCase):
    ...
    def test_get_required_detected_os_info_fields(self):
        result = windows.BaseWindowsMorphingTools.get_required_detected_os_info_fields()
    ...
    @mock.patch.object(windows.BaseWindowsMorphingTools, '_get_worker_os_drive_path')
    def test__get_dism_path(self, mock_get_worker_os_drive_path):
        ...
        self.assertEqual(result, "C:\\Windows\\System32\\dism.exe")
    ...
    def test__grant_permissions(self):
        ...
        self.conn.exec_command.assert_called_once_with(
            'icacls.exe', [mock.sentinel.path, '/grant', 'sentinel.user:(OI)(CI)F']
        )

    def test__revoke_permissions(self):
        self.morphing_tools._revoke_permissions(mock.sentinel.path, mock.sentinel.user)

        self.conn.exec_command.assert_called_once_with(
            'icacls.exe', [mock.sentinel.path, '/remove', mock.sentinel.user]
        )
    ...
    def test__add_dism_driver(self, mock_get_worker_os_drive_path):
        mock_get_worker_os_drive_path.return_value = "C:"
        result = self.morphing_tools._add_dism_driver(mock.sentinel.driver_path)
    ...
    def test__add_dism_driver_exception(self, mock_get_worker_os_drive_path):
        ...
        with self.assertLogs('coriolis.osmorphing.windows', level=logging.ERROR):
            self.assertRaises(
        ...
        self.conn.test_path.assert_called_once_with("C:\\Windows\\Logs\\DISM\\dism.log")
    ...
    def test__add_dism_driver_not_found(self, mock_get_worker_os_drive_path):
        ...
        with self.assertLogs('coriolis.osmorphing.windows', level=logging.WARN):
            self.assertRaises(
                CoriolisTestException,
                self.morphing_tools._add_dism_driver,
                mock.sentinel.driver_path,
    ...
    def test__mount_disk_image_missing_letter(self):
        self.conn.exec_ps_command.side_effect = [("", "fake stderr"), "fake-size"]
        self.assertRaises(
    ...
    def test__expand_archive(self):
        ...
        with self.assertLogs('coriolis.osmorphing.windows', level=logging.WARN):
            self.morphing_tools._expand_archive(mock.sentinel.archive_path, destination)
    ...
    def test__set_service_start_mode(self):
        self.morphing_tools._set_service_start_mode(
            mock.sentinel.key_name, mock.sentinel.service_name, mock.sentinel.start_mode
        )
        ...
        expected_command = "Set-ItemProperty -Path '%s' -Name 'Start' -Value %s" % (
            registry_path,
    ...
    def test__create_service(self):
        ...
        expected_commands = (
            ...
            % {
                "path": registry_path,
                "image_path": mock.sentinel.image_path,
    ...
    def test__delete_startup_entry(self):
        self.morphing_tools._delete_startup_entry("mock_key_name", "mock_service_name")

        registry_path = "HKLM:\\mock_key_name\\\Microsoft\\Windows\\CurrentVersion\\Run"
    ...
    def test__delete_uninstall_entry(self):
        ...
        registry_path = (
            "HKLM:\\mock_key_name\\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\*"
        )
    ...
    def test_run_user_script_raises_exception_on_write_winrm_file(
        ...
        self.assertRaises(
            exception.CoriolisException,
            self.morphing_tools.run_user_script,
            user_script,
        )
    ...
    @mock.patch.object(windows.BaseWindowsMorphingTools, '_set_service_start_mode')
    ...
    def test__disable_cloudbase_init(
        ...
        mock_set_service_start_mode.assert_called_once_with(
            ...
            windows.CLOUDBASEINIT_SERVICE_NAME,
    ...
    @mock.patch.object(windows.BaseWindowsMorphingTools, '_set_service_start_mode')
    def test__setup_existing_cbslinit_service(self, mock_set_service_start_mode):
    ...
    def test__write_local_script(self, mock_write_winrm_file):
        ...
        with mock.patch('builtins.open', mock.mock_open()) as mock_open:
            self.morphing_tools._write_local_script('C:', script_path, priority=50)
            mock_open.assert_called_once_with(script_path, 'r')
            mock_write_winrm_file.assert_called_once_with(
                self.conn, 'C:\\LocalScripts\\50-script.sh', ''
            )
    ...
    def test__write_cloudbase_init_conf(
        ...
        self.conn.exec_ps_command.assert_has_calls(
            [
                mock.call(
                    "mkdir 'C:\\Cloudbase-Init\\conf' -Force", ignore_stdout=True
                ),
                mock.call(
        ...
        conf_file_path = "C:\\Cloudbase-Init\\conf\\cloudbase-init.conf"
        conf_content = (
            "[DEFAULT]\r\n"
            "username = Admin\r\n"
    ...
    def test__write_cloudbase_init_conf_with_exception(
        ...
        self.assertRaises(
            exception.CoriolisException,
            self.morphing_tools._write_cloudbase_init_conf,
            mock.sentinel.cloudbaseinit_base_dir,
            mock.sentinel.local_base_dir,
            com_port='COM1',
            plugins=plugins,
        )
    ...
    def test__install_cloudbase_init(
        ...
        mock_check_cloudbase_init_exists,
        mock_write_cloudbase_init_conf,
        mock_unload_registry_hive,
    ):
        ...
        self.conn.exec_ps_command.assert_called_once_with(
            "mkdir '%s' -Force" % ("%s\\Log" % cloudbaseinit_base_dir),
            ignore_stdout=True,
        )
    ...
    def test_install_cloudbase_init_existing_service(
        ...
        self.morphing_tools._install_cloudbase_init(
            mock.sentinel.download_url, com_port='COM1'
        )
    ...
    def test__compile_static_ip_conf_from_registry(self):
        ...
        interfaces_reg_path = windows.INTERFACES_PATH_FORMAT % mock.sentinel.key_name
    ...
    def test_compile_static_ip_conf_from_registry_no_ip_or_subnet(self):
        ...
        interfaces_reg_path = windows.INTERFACES_PATH_FORMAT % mock.sentinel.key_name
        with self.assertLogs('coriolis.osmorphing.windows', level=logging.WARNING):
    ...
    def test_compile_static_ip_conf_from_registry_no_static_ip(self):
        ...
        with self.assertLogs('coriolis.osmorphing.windows', level=logging.DEBUG):
            self.morphing_tools._compile_static_ip_conf_from_registry(
                mock.sentinel.key_name
            )
        self.conn.exec_ps_command.assert_has_calls(
    ...
    def test__get_static_nics_info(self):
        ...
        # detected static IPs
        ips_info = [{"ip_addresses": [static_ipv4]}, {"ip_addresses": [static_ipv6]}]
        nics_info = [
        ...
        expected_result = [
            {"mac_address": "00:50:56:92:91:43", "ip_addresses": ["10.0.0.16"]},
            {
                "mac_address": "00:50:56:92:91:45",
    ...
    def test__write_static_ip_script(self, mock_write_winrm_file):
        nics_info = [{'ip_addresses': ["10.1.10.10"]}]
        ips_info = [{'ip_address': "10.1.10.10"}]
        self.morphing_tools._write_static_ip_script('C:', nics_info, ips_info)

        contents = windows.STATIC_IP_SCRIPT_TEMPLATE % {
            'nics_info': base64.b64encode(json.dumps(nics_info).encode()).decode(),
            'ips_info': base64.b64encode(json.dumps(ips_info).encode()).decode(),
        }
        mock_write_winrm_file.assert_called_once_with(
    ...
    @mock.patch.object(
        windows.BaseWindowsMorphingTools, '_compile_static_ip_conf_from_registry'
    )
    @mock.patch.object(windows.BaseWindowsMorphingTools, '_write_static_ip_script')
    @mock.patch.object(windows.BaseWindowsMorphingTools, '_get_static_nics_info')
    @mock.patch.object(windows.BaseWindowsMorphingTools, '_unload_registry_hive')
    @mock.patch.object(windows.BaseWindowsMorphingTools, '_load_registry_hive')
    @mock.patch.object(windows.uuid, 'uuid4')
    ...
    @mock.patch.object(windows.BaseWindowsMorphingTools, '_get_cbslinit_base_dir')
    @mock.patch.object(
        windows.BaseWindowsMorphingTools, '_compile_static_ip_conf_from_registry'
    )
    @mock.patch.object(windows.BaseWindowsMorphingTools, '_write_static_ip_script')
    ...
    def test_set_net_config_with_dhcp(
        ...
        result = self.morphing_tools.set_net_config(mock.sentinel.nics_info, dhcp=dhcp)
    ...
    def test_register_firstboot_script(self, mock_uuid, mock_write_winrm_file):
        ...
        self.morphing_tools.register_firstboot_script(
            mock_script, index=10, user_provided=True
        )
    ...
    @ddt.unpack
    @mock.patch.object(windows.BaseWindowsMorphingTools, "_stage_balloon_service")
    ...
    @mock.patch.object(windows.BaseWindowsMorphingTools, "_dismount_disk_image")
    def test_add_virtio_drivers(
        ...
        mock_grant_permissions.assert_called_once_with(exp_repo_path, f"*{null_sid}")
        mock_revoke_permissions.assert_called_once_with(exp_repo_path, f"*{null_sid}")
    ...
    @mock.patch.object(windows.BaseWindowsMorphingTools, "register_firstboot_script")
    ...
    @mock.patch.object(windows.BaseWindowsMorphingTools, "register_firstboot_script")
    def test_stage_balloon_service_missing_blnsvr(self, mock_register_firstboot_script):
        self.conn.test_path.return_value = False
    ...
    def test_add_virtio_drivers_failure(
        ...
        self.assertRaises(IOError, self.morphing_tools._add_virtio_drivers)
    ...
    def test_add_virtio_drivers_missing_virtio_dir(
        ...
        self.assertRaises(
            exception.CoriolisException, self.morphing_tools._add_virtio_drivers
        )
    ...
    @mock.patch.object(windows.BaseWindowsMorphingTools, "_mount_disk_image")
    def test_add_virtio_drivers_missing_iso_url(self, mock_mount_disk_image):
        self.morphing_tools._version_number = version.LooseVersion("10.0.26500")
    ...
    def test_install_dism_drivers(
        ...
        self.conn.test_path.side_effect = lambda path: path == existing_driver_path
    ...
    def test_install_dism_drivers_revokes_on_failure(
        ...
        self.assertRaises(
            IOError, self.morphing_tools._install_dism_drivers, ["e:\\NetKVM"]
        )
    ...
    def test_setup_qemu_agent_installation_local_script_from_qemu_ga_dir(
        ...
        exp_script = windows.QEMU_GUEST_AGENT_INSTALL_FROM_DIR_SCRIPT_FORMAT % {
